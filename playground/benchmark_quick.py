...
from quick.awq.models.base import BaseAWQForCausalLM
...
def run_round(generator, model_path, quant_file, n_generate, input_ids, batch_size, no_safetensors, pretrained):
    ...
    if pretrained:
        ...
        model = AutoAWQForCausalLM.from_quantized(
            model_path, quant_file,max_new_tokens=n_generate, 
            batch_size=batch_size, safetensors=not no_safetensors
        )
...
if __name__ == "__main__":
    ...
    for bs in batch_sizes:
        args.batch_size = bs
        rez.append(main(args))

    rez = pd.concat(rez)
    rez["Model"] = args.model_path
    rez["Quantized"] = True if not args.pretrained else False
    header = ["Model", "Quantized"]
    rez = rez[header + [c for c in rez.columns if c not in header]]
    rez.loc[rez["Prefill tokens/s"] == "OOM", "Prefill tokens/s"] = math.nan
    rez.loc[rez["Decode tokens/s"] == "OOM", "Decode tokens/s"] = math.nan
